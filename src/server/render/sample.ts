...
export function sampleStates(): RenderState[] {
  const base: RenderState = {
    ...
    tiles: Array.from({ length: 100 }, (_, idx) => {
      ...
      if (x === 1 && y === 1) return { terrain: "hq", owner: "P1" };
      if (x === 8 && y === 8) return { terrain: "hq", owner: "P2" };
      if ((x + y) % 5 === 0) return { terrain: "city", owner: null };
